...
function removeFirstOccurrences(str, value) {
  ...
  let newStr = str;
  if (substrIndex !== -1)
    newStr = str.slice(0, substrIndex) + str.slice(substrIndex + value.length);
  return newStr;
}
...
function invertCase(str) {
  ...
  let invertedStr = '';
  arr.forEach((char) => {
    if (char === char.toLowerCase()) invertedStr += char.toUpperCase();
    else invertedStr += char.toLowerCase();
  });
  return invertedStr;
}
